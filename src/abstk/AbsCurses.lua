...
function AbsCursesButton:draw(drawable, x, y, focus)
  drawable:mvaddstr(y, x, self.label, colors.button)
  if focus then
    local left, right = ">", "<"
...
function Screen:run()
  ...
  while true do
    --local title = AbsCursesLabel.new(self.title)
    --title:draw(stdscr, 3, 2)
    self.pad:mvaddstr(1, 2, self.title)
    ...
    for i, item in ipairs(self.widgets) do
      ...
      -- TODO drawable pad (curses.newpad()), como uma div para tratar scroll
      item.widget:draw(stdscr, 5, y)
